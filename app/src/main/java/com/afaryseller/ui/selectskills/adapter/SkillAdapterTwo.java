...
public class SkillAdapterTwo extends RecyclerView.Adapter<SkillAdapterTwo.MyViewHolder> {
    Context context;
    ArrayList<SkillModel.TravelTicket> arrayList;
    SkillsListener listener;

    public SkillAdapterTwo(Context context, ArrayList<SkillModel.TravelTicket> arrayList, SkillsListener listener) {
        ...
        public MyViewHolder(@NonNull ItemAddSkillsBinding itemView) {
            ...
            binding.addSkillText.setOnClickListener(v -> {
                if(arrayList.get(getAdapterPosition()).getStatus().equals("0")) {
                    ...
                    listener.skills("travel",arrayList.get(getAdapterPosition()).getId(),arrayList.get(getAdapterPosition()).getName(),"1",getAdapterPosition());
                }
                else {
                    arrayList.get(getAdapterPosition()).setStatus("0");
                    listener.skills("travel",arrayList.get(getAdapterPosition()).getId(),arrayList.get(getAdapterPosition()).getName(),"0",getAdapterPosition());
