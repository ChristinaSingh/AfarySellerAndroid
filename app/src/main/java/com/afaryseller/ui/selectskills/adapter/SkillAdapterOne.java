...
public class SkillAdapterOne extends RecyclerView.Adapter<SkillAdapterOne.MyViewHolder> {
    Context context;
    ArrayList<SkillModel.Result>arrayList;
    SkillsListener listener;
    public SkillAdapterOne(Context context, ArrayList<SkillModel.Result> arrayList,SkillsListener listener) {
        ...
        public MyViewHolder(@NonNull ItemAddSkillsBinding itemView) {
            ...
            binding.addSkillText.setOnClickListener(v -> {
                if(arrayList.get(getAdapterPosition()).getStatus().equals("0")) {
                    ...
                    listener.skills("main",arrayList.get(getAdapterPosition()).getId(),arrayList.get(getAdapterPosition()).getName(),"1",getAdapterPosition());

                }
                else {
                    arrayList.get(getAdapterPosition()).setStatus("0");
                    listener.skills("main",arrayList.get(getAdapterPosition()).getId(),arrayList.get(getAdapterPosition()).getName(),"0",getAdapterPosition());
