...
public class SkillAdapterFour extends RecyclerView.Adapter<SkillAdapterFour.MyViewHolder> {
    Context context;
    ArrayList<SkillModel.HomeService> arrayList;
    SkillsListener listener;
    public SkillAdapterFour(Context context, ArrayList<SkillModel.HomeService> arrayList,SkillsListener listener) {
        ...
        public MyViewHolder(@NonNull ItemAddSkillsBinding itemView) {
            ...
            binding.addSkillText.setOnClickListener(v -> {
                if(arrayList.get(getAdapterPosition()).getStatus().equals("0")) {
                    ...
                    listener.skills("home",arrayList.get(getAdapterPosition()).getId(),arrayList.get(getAdapterPosition()).getName(),"1",getAdapterPosition());

                }
                else{
                    arrayList.get(getAdapterPosition()).setStatus("0");
                    listener.skills("home",arrayList.get(getAdapterPosition()).getId(),arrayList.get(getAdapterPosition()).getName(),"0",getAdapterPosition());
