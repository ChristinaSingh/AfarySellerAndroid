...
public class EditTimeAdapter extends RecyclerView.Adapter<EditTimeAdapter.MyViewHolder> {
    Context context;
    ArrayList<OpenTimeModel.Result> arrayList;
    AddDateLister listener;

    public EditTimeAdapter(Context context, ArrayList<OpenTimeModel.Result> arrayList,AddDateLister listener) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        if(!arrayList.get(position).getOpenTime().equals("")) holder.binding.tvTime.setText(arrayList.get(position).getWeekName() + "\n" + arrayList.get(position).getOpenTime());
